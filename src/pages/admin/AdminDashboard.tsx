...
import { Package, ShoppingCart, IndianRupee, Clock, CreditCard, Scissors } from "lucide-react";

const AdminDashboard = () => {
  const stats = useAdminStats();
  const [extra, setExtra] = useState({ paidRevenue: 0, paidCount: 0, pendingPayments: 0, tailoringPending: 0 });

  useEffect(() => {
    (async () => {
      const [paidRes, tailRes] = await Promise.all([
